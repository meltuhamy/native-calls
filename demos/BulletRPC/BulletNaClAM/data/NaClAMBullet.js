...
function NaClAMBulletStepSceneHandler(msg) {
  ...
  if (msg.header.cmd == 'sceneupdate') {
    ...
    NCStats.total_simTime += simTime;
    if(window.NaClAMSentStart != undefined){
      NCStats.total_tripTime += (new Date()).getTime() - window.NaClAMSentStart;
      window.NaClAMSentStart = undefined;
    }
    NCStats.num_trips++;
    NCStats.num_sims++;
